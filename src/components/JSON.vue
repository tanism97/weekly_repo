...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(time, day) in bookstores.openingHours" :key="day">{{ day }} : {{ time }}</li>
      </ul>
      ...
      <p>We operate in:</p>
      <ul>
        <li v-for="place in bookstores.countries" :key="place">
          {{ place }}
        </li>
      </ul>
      <p>Our #1 seller:</p>
...
.lab-section {
  ...
  margin-bottom: 20px;
  border-radius: 8px;
  box-shadow: 0 2px 4px rgba(239, 236, 236, 0.1);
}
